  ...
  (let
      [idx (atom 1) 
       ...
       (reset! elStr (str "{\"name\":\"" (.substring tagStr @idx
               (.indexOf tagStr (sym-log.cljs.string.fetch-first-in-string tagStr markers @idx) @idx))
       ...
       (if (= 0 @idx) nil;
        (loop [strng (.substring
                    tagStr @idx
                   (.indexOf tagStr (sym-log.cljs.string.fetch-first-in-string tagStr markers (inc @idx)) @idx))
              ]
